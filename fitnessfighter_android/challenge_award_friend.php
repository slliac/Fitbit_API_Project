...
if($result)
{
  ...
  $result = mysqli_query($db->con, "SELECT COUNT(*) as size FROM challenge WHERE challenge_id = '$challenge_id'") or die(mysqli_error($db->con));


  if($result)
  {
    while ($row = mysqli_fetch_array($result)) {
       $award = $award * $row["size"];
    }

      $result = mysqli_query($db->con, "UPDATE challenge SET complete = '1' WHERE challenge_id = '$challenge_id'") or die(mysqli_error($db->con));
      $result = mysqli_query($db->con, "UPDATE game_family SET money = money + '$award' WHERE familyName = '$familyName'") or die(mysqli_error($db->con));
