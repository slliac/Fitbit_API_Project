...
if(true)
{
  $personal_background = "iVBORw0KGgoAAAANSUhEUgAAA4MAAAF2CAIAAABbNdouAAAAA3NCSVQICAjb4U/gAAAgAElEQVR4
...
$result = mysqli_query($db->con, "SELECT * FROM user_detail WHERE email = '$email'") or die(mysqli_error($db->con));


// check for empty result
if (mysqli_num_rows($result) <= 0)
{
  $result = mysqli_query($db->con, "INSERT INTO user_detail(fb_user_id, name, gender, user_job, dob ,weight, height, message, fb_access_token, email, last_login_date, profile_image, memberSince, gcm_token, background_image) VALUES('$fb_user_id', '$name', '$gender', '$job' , '$dob' ,  '$weight', '$height', '$info', '$fb_access_token', '$email', '$last_login_date', '$profile_image', '$memberSince', '$gcm_token', '$personal_background')") or die(mysqli_error($db->con));

  $result = mysqli_query($db->con, "INSERT INTO user(name, gender, lastsync, dob, user_job, weight, height, message, fb_user_id, fb_access_token) VALUES('$name', '$gender', null, '$dob', '$job', '$weight', '$height', '$info', '$fb_user_id', '$fb_access_token')") or die(mysqli_error($db->con));
